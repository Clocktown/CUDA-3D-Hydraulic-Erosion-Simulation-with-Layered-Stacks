...
inline void LightingSystem::updateUniformBuffer(const entt::exclude_t<Excludes...> excludes)
{
	...
	Lighting& lighting{ scene.addSingleton<Lighting>() };
	...
	{
		const auto view{ scene.getView<LocalToWorld, PointLight, Includes...>(excludes) };
	...
	{
		const auto view{ scene.getView<LocalToWorld, SpotLight, Includes...>(excludes) };
	...
	{
		const auto view{ scene.getView<LocalToWorld, DirectionalLight, Includes...>(excludes) };
