...
void VertexArray::create(const Span<const VertexAttribute>&& vertexAttributes)
{
	...
	for (int i{ 0 }; i < vertexAttributes.getCount(); ++i)
	{
		...
		GL_CHECK_ERROR(glVertexArrayAttribFormat(handle, location, vertexAttribute.count, vertexAttribute.type, vertexAttribute.normalized, static_cast<GLuint>(vertexAttribute.relativeOffset)));
