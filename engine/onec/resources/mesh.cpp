...
Mesh::Mesh(const std::filesystem::path& file, const unsigned int flags, const bool cudaAccess)
{
	...
	indexBuffer.initialize(asBytes(indices), cudaAccess);
	positionBuffer.initialize(asBytes(positions), cudaAccess);
	vertexPropertyBuffer.initialize(asBytes(vertexProperties), cudaAccess);
